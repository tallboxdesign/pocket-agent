...
1. **GLM tiered model system** — `glmBulk()` for email classification throughput (default `glm-4.7-flash`, configurable to `glm-4.7-flashx` when paid plan available); `glmFlash()` kept for quality-sensitive tasks (draft replies, digests, label refinement). Default `glmConcurrency` raised to 3. Health check pings both models (skips duplicate if same).
